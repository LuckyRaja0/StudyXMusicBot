...
from StudyXMusic import YouTube, app
from StudyXMusic.core.call import StudyX
from StudyXMusic.misc import SUDOERS, db
from StudyXMusic.utils.database import (is_active_chat,
                                       is_music_playing, is_muted,
                                       is_nonadmin_chat, music_off,
                                       music_on, mute_off, mute_on,
                                       set_loop)
from StudyXMusic.utils.decorators.language import languageCB
from StudyXMusic.utils.formatters import seconds_to_min
from StudyXMusic.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
from StudyXMusic.utils.stream.autoclear import auto_clean
from StudyXMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await StudyX.pause_stream(chat_id)
        ...
        await StudyX.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await StudyX.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await StudyX.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await StudyX.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await StudyX.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
